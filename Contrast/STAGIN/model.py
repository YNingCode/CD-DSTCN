...
num_nodes = 90
time_points = 195
feature_dim = 1  # 因为时序数据被视为时间维度，因此特征维度为1
dim = (20, 90, 195)
dim_2 = (20, 90, 90)
a = torch.randn(dim_2)
v = torch.randn(dim)
# 将输入张量转换为符合 ModelSTAGIN 输入格式的形式
v = v.permute(0, 2, 1).unsqueeze(-1)  # 转换为 (batch_size, time_points, num_nodes, feature_dim)
a = a.unsqueeze(1).expand(-1, time_points, -1, -1)  # 转换为 (batch_size, time_points, num_nodes, num_nodes)
# 生成时间戳 t
t = torch.arange(time_points).unsqueeze(-1).float()  # 调整时间戳输入的形状
 # 调整时间戳输入的形状
# 定义采样终点 sampling_endpoints
sampling_endpoints = [time_points]  # 使用所有时间点
# 创建 ModelSTAGIN 实例
...
sparsity = 20
dropout = 0.5
model = Model(input_dim, hidden_dim, num_classes, num_heads, num_layers, sparsity, dropout)
out,_,_,_ = model(v, a, t, sampling_endpoints)
print(out.shape)