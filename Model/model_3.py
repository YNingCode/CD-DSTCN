...
class STBlock(nn.Module):
    def __init__(self, gcn_input, gcn_out):
        ...
        self.linear = nn.Linear(30,gcn_input)
    ...
    def forward(self, fdata, g_before):
        ...
        # CNN
        fdata = fdata.unsqueeze(2) # 20，90，1，65
...
class Model(nn.Module):
    ...
    def forward(self, fdata):
        ...
        input = reshape_dot(input, temporal_Att)

        fdata = input.permute(1,0,2,3)
        all_out= []
        all_adj = []
        shape = (fdata[0].size(0),90,30)
        g_before = torch.zeros(shape).to(fdata.device)
        # g_before_2 = torch.zeros(shape).to(fdata.device)
        # g_before_3 = torch.zeros(shape).to(fdata.device)
        for t in range(fdata.size(0)):
            out, g_before, adj = self.st_1(fdata[t], g_before)
            # out, g_before_2 = self.st_2(out, g_before_2)
            # out, g_before_3 = self.st_3(out, g_before_3)
            all_out.append(out)
            all_adj.append(adj)
        ...
        # 不加conf，直接相加
        # all_out = all_out.permute(1, 0, 2, 3)
        # final = torch.sum(all_out, dim=1)

        # out = F.relu(g_out)
        block_out = self.f1(final)
        block_outs = self.d1(self.bn1(self.l1(block_out)))
