...
class cheb_conv_withSAt(nn.Module):
    ...
    def __init__(self, K, in_channels, out_channels):
        ...
        self.Theta = nn.ParameterList([nn.init.uniform_(nn.Parameter(torch.FloatTensor(in_channels, out_channels))) for _ in range(K)])

    def forward(self, x, spatial_attention, adj):
        '''
        ...
        for time_step in range(num_of_timesteps):
            ...
            for k in range(self.K):
                ...
                output = output.to(rhs.device) + rhs.matmul(theta_k)  # (b, N, F_in)(F_in, F_out) = (b, N, F_out)
...
class Temporal_Attention_layer(nn.Module):
    ...
    def forward(self, x):
        ...
        rhs = torch.matmul(self.U3, x) + 1e-10  # (F)(B,N,F,T)->(B, N, T)
...
class ASTGCN_block(nn.Module):
    ...
    def __init__(self, in_channels, K, nb_chev_filter, nb_time_filter, time_strides, num_of_vertices, num_of_timesteps):
        ...
        self.cheb_conv_SAt = cheb_conv_withSAt(K, in_channels, nb_chev_filter)
    ...
    def forward(self, x, adj):
        ...
        # cheb gcn
        spatial_gcn = self.cheb_conv_SAt(x, spatial_At, adj)  # (b,N,F,T)
